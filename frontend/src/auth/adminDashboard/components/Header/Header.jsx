          ...
          <div className="relative" ref={dropdownRef}>
            ...
            >
              ...
              <div className="hidden sm:block text-left">
                <p className="text-xs font-semibold text-text-primary leading-tight">{user?.name || "Admin"}</p>
                <p className="text-[10px] text-text-muted leading-tight capitalize">{user?.role || "admin"}</p>
              </div>
              ...
              {dropdownOpen && (
                ...
                >
                  <div className="px-3 py-2.5 border-b border-border-color">
                    <p className="text-xs font-semibold text-text-primary">{user?.name || "Admin"}</p>
                    <p className="text-[10px] text-text-muted mt-0.5 truncate">{user?.email || "admin@campusync.com"}</p>
                  </div>
                  <div className="py-1">
                    <DropdownItem icon={<LayoutDashboard size={14} />} label="Dashboard" to="/admin/dashboard" onClick={() => setDropdownOpen(false)} />
                    <DropdownItem icon={<User size={14} />} label="Profile" to="/admin/profile" onClick={() => setDropdownOpen(false)} />
                    <DropdownItem icon={<Settings size={14} />} label="Settings" to="/admin/settings" onClick={() => setDropdownOpen(false)} />
                  </div>
