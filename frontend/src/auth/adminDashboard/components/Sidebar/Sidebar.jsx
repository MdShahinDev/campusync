...
const navGroups = [
  ...
  },
  {
    ...
    items: [
      ...
      { name: "Pending Users", path: "/admin/pending-users", icon: Clock },
      { name: "New User", path: "/admin/new-user", icon: UserPlus },
    ],
  },
  {
          ...
          <div className={`flex items-center gap-2.5 px-2.5 py-2 rounded-lg ${collapsed ? "justify-center" : ""}`}>
            ...
            <AnimatePresence>
              {!collapsed && (
                ...
                >
                  <p className="text-xs font-semibold text-text-primary truncate">{user?.name || "Admin"}</p>
                  <p className="text-[10px] text-text-muted truncate">{user?.email || "admin@campusync.com"}</p>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
