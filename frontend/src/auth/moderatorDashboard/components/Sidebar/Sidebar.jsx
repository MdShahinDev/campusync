...
const navGroups = [
  {
    label: "Overview",
    items: [
      { name: "Dashboard", path: "/moderator/dashboard", icon: LayoutDashboard },
    ],
  },
  {
    label: "Users",
    items: [
      { name: "All Users", path: "/moderator/all-users", icon: Users },
      { name: "Pending Users", path: "/moderator/pending-users", icon: Clock },
    ],
  },
  {
  ...
  },
  {
    label: "Account",
    items: [
      { name: "Messaging", path: "/moderator/messaging", icon: MessagesSquare, badge: "unread" },
      { name: "Profile", path: "/moderator/profile", icon: User },
    ],
  },
];
...
export default function Sidebar({ isOpen, onClose, collapsed, onToggleCollapse }) {
  ...
  const isItemActive = (item) =>
    ...
    (item.path !== "/moderator/dashboard" && location.pathname.startsWith(item.path));
          ...
          <div className={`flex items-center gap-2.5 px-2.5 py-2 rounded-lg ${collapsed ? "justify-center" : ""}`}>
            ...
            <AnimatePresence>
              {!collapsed && (
                ...
                >
                  <p className="text-xs font-semibold text-text-primary truncate">{user?.name || "Moderator"}</p>
                  <p className="text-[10px] text-text-muted truncate">{user?.email || "moderator@campusync.com"}</p>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
